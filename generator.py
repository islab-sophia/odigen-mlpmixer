...
class MLPLayerBlock(nn.Module):
    def __init__(self,in_channel,layer,kernel=3,use_patch=False,single=False):
        ...
        if not use_patch:
            self.mixer = MLPCBNMixer(layer=layer, dropout=0, channel_size=in_channel) # MLPMixerBlock
        else:
            self.mixer = MLPCBNDepthWiseMixer(layer=layer, channel_size=in_channel,kernel=kernel,single=single) # DepthwiseBlock
...
class Generator(nn.Module):
    def __init__(self,channel=512,layer=[4,4,4,4,4],activation=True,dw_kernel=5,single=False,out_kernel=1):
        ...
        self.mixer1 = MLPLayerBlock(channel, layer[0]) # MLPMixerBlock
        self.mixer2 = MLPLayerBlock(channel // 4, layer[1], use_patch=True, kernel=dw_kernel,single=single) # DeptiwiseBlock
...
if __name__ == '__main__':
    ...
    module = Generator(single=False).cuda()
